...
export default function EstimatorClient({ fxRate, fxDate }: { fxRate: number; fxDate: string }) {
  ...
  // ---- BOQ document in the GMHS proposal-template format ----
  function downloadBOQ() {
    const money = (n: number) => `₹${n.toLocaleString("en-IN")}`;
    const stageBlocks = result.stages.map((s) => {
      if (s.lines) {
        const rows = s.lines.map((l) => `<tr><td style="padding:7px 10px;border-bottom:1px solid #eee;">${l.item}</td><td style="text-align:right;padding:7px 10px;border-bottom:1px solid #eee;">${l.qty.toLocaleString("en-IN")} ${l.unit}</td><td style="text-align:right;padding:7px 10px;border-bottom:1px solid #eee;">${money(l.rateINR)}</td><td style="text-align:right;padding:7px 10px;border-bottom:1px solid #eee;">${money(l.amountINR)}</td></tr>`).join("");
        return `<h3 style="margin:18px 0 6px;color:#0a1424;">${s.label} — ${formatINR(s.subtotalINR)}</h3>
          <table style="width:100%;border-collapse:collapse;font-size:12px;"><tr style="background:#0a1424;color:#fff;"><th style="text-align:left;padding:6px 10px;">Item</th><th style="text-align:right;padding:6px 10px;">Qty</th><th style="text-align:right;padding:6px 10px;">Rate</th><th style="text-align:right;padding:6px 10px;">Amount</th></tr>${rows}</table>`;
      }
      // automation sections (proposal format)
      const secHtml = (s.sections || []).map((sec) => {
        const rows = sec.lines.map((l, i) => `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee;">${i + 1}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;">${l.make || ""}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;">${l.item}</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">${l.qty}</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">${money(l.rateINR)}</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">${money(l.amountINR)}</td></tr>`).join("");
        const mrow = (k: string, v: string, b = false) => `<tr><td colspan="5" style="text-align:right;padding:4px 10px;${b ? "font-weight:700;background:#f7f3e8;" : ""}">${k}</td><td style="text-align:right;padding:4px 10px;${b ? "font-weight:700;background:#f7f3e8;" : ""}">${v}</td></tr>`;
        return `<h4 style="margin:14px 0 4px;color:#B8902F;">${sec.name}</h4>
          <table style="width:100%;border-collapse:collapse;font-size:12px;"><tr style="background:#0a1424;color:#fff;"><th style="text-align:left;padding:6px 10px;">#</th><th style="text-align:left;padding:6px 10px;">Make</th><th style="text-align:left;padding:6px 10px;">Description</th><th style="text-align:right;padding:6px 10px;">Qty</th><th style="text-align:right;padding:6px 10px;">Rate</th><th style="text-align:right;padding:6px 10px;">Amount</th></tr>
          ${rows}
          ${mrow("Pre-tax amount", money(sec.listINR))}
          ${mrow("Discount (30%)", "– " + money(sec.discountINR))}
          ${mrow("Nett amount", money(sec.nettINR))}
          ${mrow("GST @18%", money(sec.gstINR))}
          ${mrow("Installation", money(sec.installINR))}
          ${mrow("Total", formatINR(sec.totalINR), true)}
          </table>`;
      }).join("");
      return `<h3 style="margin:20px 0 4px;color:#0a1424;">${s.label} — ${formatINR(s.subtotalINR)}</h3>${secHtml}`;
    }).join("");

    const summaryRows = result.stages.map((s) => `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee;">${s.label}</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">${s.durationMin}–${s.durationMax} mo</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">${formatINR(s.subtotalINR)}</td><td style="text-align:right;padding:6px 10px;border-bottom:1px solid #eee;">~${usd(s.subtotalINR)}</td></tr>`).join("");
    ...
    <body style="font-family:Arial,sans-serif;max-width:880px;margin:24px auto;color:#1a2744;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;"><tr style="background:#0a1424;color:#fff;"><th style="text-align:left;padding:7px 10px;">Stage</th><th style="text-align:right;padding:7px 10px;">Duration</th><th style="text-align:right;padding:7px 10px;">Amount (₹)</th><th style="text-align:right;padding:7px 10px;">~ US$</th></tr>${summaryRows}
      <tr style="background:#f7f3e8;font-weight:700;"><td style="padding:8px 10px;">Estimated total</td><td></td><td style="text-align:right;padding:8px 10px;">${formatINR(result.lowINR)} – ${formatINR(result.highINR)}</td><td style="text-align:right;padding:8px 10px;">~${usd(result.lowINR)}–${usd(result.highINR)}</td></tr></table>
      ${stageBlocks}
      <p style="font-size:11px;color:#888;margin-top:18px;line-height:1.5;">Automation prices follow the GMHS proposal format (list → 30% discount → +18% GST → installation). Construction/interiors are indicative turnkey rates (GST extra). USD converted at ₹${fxRate}/$ on ${fxDate}, indicative. Final binding quote follows a site visit &amp; engineer review. © Grow More Solutions.</p>
    ...
    <div className="grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="glass-card rounded-xl p-6">
              ...
              <div key={s.id}>
                ...
                {s.sections?.map((sec) => (
                  <div key={sec.name} className="mt-2 mb-3">
                    <p className="text-xs font-semibold text-navy-200 mt-2 mb-1">{sec.name} <span className="text-navy-400 font-normal">— {formatINR(sec.totalINR)}</span></p>
                    {sec.lines.map((l, i) => (
                      <div key={i} className="flex justify-between text-xs text-navy-300 py-1 border-b border-navy-800/60">
                        <span className="pr-3">{l.make && <span className="text-navy-500">{l.make} · </span>}{l.item} <span className="text-navy-500">× {l.qty}</span></span>
                        <span className="whitespace-nowrap">{formatINR(l.amountINR)}</span>
                      </div>
                    ))}
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-navy-500 mt-1">
                      <span>List {formatINR(sec.listINR)}</span><span>· −30% {formatINR(sec.discountINR)}</span><span>· GST {formatINR(sec.gstINR)}</span><span>· Install {formatINR(sec.installINR)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <p className="text-[11px] text-navy-500 mt-4">Automation follows the GMHS proposal format (incl. 30% discount, 18% GST &amp; installation). Construction/interiors are indicative turnkey rates, GST extra.</p>
        </div>
      </div>
      ...
      <div className="lg:col-span-1">
        <div className="lg:sticky lg:top-24 space-y-5">
          <div className="glass-card rounded-xl p-6">
            ...
            <p className="text-[11px] text-navy-500 mt-2">Indicative · ₹{fxRate}/$ on {fxDate}</p>
